    ...
    try:
        ...
        if not stat.S_ISREG(os.fstat(secret_fd).st_mode):
            source_error = OSError(
                f"Generated secret path is not a regular file: {path}"
            )
            msg = (
                f"Cannot create or read the generated secret key at {path}. "
                "Make FLOPPY_DATA_DIR writable or set SECRET."
            )
            raise ImproperlyConfigured(msg) from source_error
        os.fchmod(secret_fd, 0o600)

        if secret_created:
            secret_file = os.fdopen(secret_fd, "w", encoding="utf-8")
            secret_fd = None
            with secret_file:
                secret_file.write(value)
            return value, True
        secret_file = os.fdopen(secret_fd, "r", encoding="utf-8")
        secret_fd = None
        with secret_file:
            return secret_file.read().strip(), False

...
class DataPathSettingsTests(SimpleTestCase):
    ...
    def test_generated_secret_rejects_non_regular_file_before_chmod(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(stat.S_IMODE(secret_path.stat().st_mode), 0o755)
...
""",
    ...
    def test_paths_with_spaces_and_leading_dash_remain_single_arguments(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn(
                "python <-c> <from config.sqlite_integrity import "
                "check_database_integrity; import sys; "
                f"check_database_integrity(sys.argv[1])> <{database_path}>",
                commands,
            )
    ...
    def test_corrupt_custom_database_stops_before_migrations(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            data_dir = root / "data"
            database_path = root / "database" / "db.sqlite3"

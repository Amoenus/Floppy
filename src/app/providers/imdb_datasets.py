...
def download_videogame_title_index() -> dict[str, tuple[str, int | None]]:
    ...
    with urllib.request.urlopen(TITLE_BASICS_URL, timeout=_DOWNLOAD_TIMEOUT) as resp:  # noqa: S310
...
def download_principals(tconsts: set[str]) -> dict[str, list[dict]]:
    ...
    with urllib.request.urlopen(PRINCIPALS_URL, timeout=_DOWNLOAD_TIMEOUT) as resp:  # noqa: S310
        with gzip.open(resp, "rt", encoding="utf-8") as f:
            reader = csv.DictReader(f, delimiter="\t")
            for row in reader:
                tconst = row.get("tconst", "").strip()
...
def download_names(nconsts: set[str]) -> dict[str, str]:
    ...
    with urllib.request.urlopen(NAME_BASICS_URL, timeout=_DOWNLOAD_TIMEOUT) as resp:  # noqa: S310
        with gzip.open(resp, "rt", encoding="utf-8") as f:
            reader = csv.DictReader(f, delimiter="\t")
            for row in reader:
                nconst = row.get("nconst", "").strip()
...
def _clean_optional(value: str | None) -> str:
    """IMDB datasets use the literal string "\\N" for nulls; normalize to ""."""

# Generated by Django 5.2.16 on 2026-08-17

from django.db import migrations

# The placeholder previously hotlinked TMDB's glyphicons SVG. Existing rows
# stored that URL in their image column. Rewrite them to the embedded data-URI
# placeholder that replaced it so they are recognised as "no image" again.
OLD_IMG_NONE = (
    "https://www.themoviedb.org/assets/2/v4/glyphicons/basic/"
    "glyphicons-basic-38-picture-grey-c2ebdbb057f2a7614185931650f8cee23fa137b93812ccb132b9df511df1cfac.svg"
)
NEW_IMG_NONE = (
    "data:image/svg+xml;base64,"
    ""
)


def _image_field_can_store_placeholder(model):
    """Return whether the historical image field can store the new sentinel."""
    max_length = model._meta.get_field("image").max_length
    return max_length is None or len(NEW_IMG_NONE) <= max_length


def rewrite_old_placeholder(apps, schema_editor):
    if NEW_IMG_NONE == OLD_IMG_NONE:
        return

    for model_name in ("Item", "Person", "Artist", "Album", "PodcastShow"):
        model = apps.get_model("app", model_name)

        # PostgreSQL enforces the historical URLField varchar(200) limit. Do not
        # write the longer data URI until a later schema migration has widened
        # the field. The follow-up migration retries this rewrite after widening.
        if not _image_field_can_store_placeholder(model):
            continue

        model.objects.filter(image=OLD_IMG_NONE).update(image=NEW_IMG_NONE)


class Migration(migrations.Migration):

    dependencies = [
        ("app", "0154_episode_watch_operation_id"),
    ]

    operations = [
        migrations.RunPython(rewrite_old_placeholder, migrations.RunPython.noop),
    ]
